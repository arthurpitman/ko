...
mkdir -p "$GOPATH/src/github.com/arthurpitman/ko"
cp -r "$ROOT_DIR/"* "$GOPATH/src/github.com/arthurpitman/ko/"

pushd "$GOPATH/src/github.com/arthurpitman/ko" || exit 1
...
RESULT="$(./ko build --local --platform="linux/$GOARCH" "$GOPATH/src/github.com/arthurpitman/ko/test" | grep "$FILTER" | xargs -I% docker run %)"
...
RESULT="$(KO_FLAGS="--platform=badvalue" ./ko build --local --platform="linux/$GOARCH" "$GOPATH/src/github.com/arthurpitman/ko/test" | grep "$FILTER" | xargs -I% docker run %)"
if [[ "$RESULT" != *"Hello there"* ]]; then
  echo "Test FAILED. Saw $RESULT" && exit 1
else
  echo "Test PASSED"
